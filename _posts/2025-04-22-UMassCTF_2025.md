...
If we look carefully, we can see that the `default-src` is a typo, it should be `default-src`. This means that we only need to consider the `connect-src` directive. From ChatGPT:
...
I solve this challenge after the CTF ends. The challenge is a simple page where we can register an account, login, create a python game and test it. There are 2 things that I first notice:
...
1. We can import libraries but almost functions are blocked such as bultin functions like `print`, `str`, `len`,... or functions that from imported libraries like `os`, `sys`, `subprocess`,...
2. Any function that begins with `_` is also blocked. 
3. Whenever we have a function named `eval` or `exec`, it is banned (sound ridiculous but true, you can find this by analyze the library).
4. However, there's a magic function called `getattr`. From ChatGPT: `The purpose of getattr in Python is to dynamically access an attribute of an object using its name as a string. This is especially useful when you don’t know the name of the attribute until runtime.`. As the attribute is loaded at runtime, we can use this to bypass the restriction. This function is not blocked in RestrictedPython and it also not starts with `_`.
...
4. My idea is to use `echo` command to write the flag to an exposed endpoint, which is `/app/static/flag`. However, there's even a better way to do this. This is the payload that I come up with when surfing the CTF's discord channel:
...
Sounds unbelievable, but it works! Functions named `exec` and `eval` are banned, but if we import like that, when the payload get to `ast.parse`, the function named (or actually the func.id) id parse as `e`. You heard right, that's literally `e`.
